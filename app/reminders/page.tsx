...
export default function RemindersPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          showNotification(t("reminders.error.description"), "error")
        }
      } else {
        setLoadingError(true)
        showNotification(t("reminders.error.description"), "error")
  ...
  // Получение локали для date-fns в зависимости от текущего языка
  const getDateLocale = () => {
    switch (language) {
      case "en":
        return undefined // date-fns использует en по умолчанию
      case "kz":
        // Для казахского языка можно использовать русскую локаль как ближайшую
        return ru
      case "ru":
      default:
        return ru
  ...
  const groupedReminders = useMemo(() => {
    ...
    sortedReminders.forEach((reminder) => {
      ...
      if (isToday(reminder.date)) {
        dateKey = "today"
        dateTitle = t("reminders.today")
      } else if (isTomorrow(reminder.date)) {
        dateKey = "tomorrow"
        dateTitle = t("reminders.tomorrow")
      } else if (isYesterday(reminder.date)) {
        dateKey = "yesterday"
        dateTitle = t("reminders.yesterday")
      } else {
        dateKey = format(reminder.date, "yyyy-MM-dd")
        dateTitle = format(reminder.date, "d MMMM yyyy", { locale: getDateLocale() })
      }
      ...
      <div className="flex-1 p-4 md:p-8 w-full">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">{t("reminders.error.title")}</h2>
            <p className="text-muted-foreground max-w-md mb-6">{t("reminders.error.description")}</p>
            <Button onClick={() => window.location.reload()} className="bg-amber-600 hover:bg-amber-700">
              {t("reminders.error.refresh")}
            </Button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">{t("reminders.empty.title")}</h2>
            <p className="text-muted-foreground max-w-md mb-6">
              {searchQuery ? t("reminders.empty.search") : t("reminders.empty.description")}
            </p>
            <Button
              onClick={handleCreateReminder}
